...
describe('CustomIntegration', function () {
  ...
  beforeEach(async () => {
    ({ curvePoolIntegration, signer1, signer2, signer3, yearnVaultRegistry, babController } = await setupTests()());
  });
  ...
  describe('testing custom yearn example', function () {
    pick(GARDENS).forEach(({ token, name }) => {
      ...
      it(`uses test a chained custom integration in a ${name} garden`, async function () {
        await transferFunds(token);
        await fund([signer1.address], { tokens: [token] });
        // Enable custom integrations
        const garden = await createGarden({ reserveAsset: token, customIntegrationsEnabled: true });
        ...
        // 3Crypto Yearn vault
        const integrations = [curvePoolIntegration.address, deployment.address];
        ...
        await executeStrategy(strategyContract, { amount });
        const integrationInstance = await ethers.getContractAt('ICustomIntegration', deployment.address);
        const resultToken = await getERC20(
          await integrationInstance.getResultToken('0xE537B5cc158EB71037D4125BDD7538421981E6AA'),
        );
        // Check NAV
        const nav = await strategyContract.getNAV();
        expect(nav).to.be.closeTo(amount, amount.div(30));
