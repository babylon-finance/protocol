...
describe('governor', function () {
  ...
  it.only('check failing strategy', async function () {
    const keeper = await impersonateAddress('0x74D206186B84d4c2dAFeBD9Fd230878EC161d5B8');
    const timelockController = await impersonateAddress('0xe6ed0eacb79a6e457416e4df38ed778fd6c6d193');
    const controller = await ethers.getContractAt('BabController', '0xd4a5b5fcb561daf3adf86f8477555b92fba43b5f', timelockController);
    ...
    for (const { contract, type, operation, args } of [
      ...
      if (type === 'operation') {
        await controller.connect(timelockController).setOperation(operation, deployment.address, { gasPrice: 0 });
      }
    }
    ...
    await controller.connect(timelockController).setMasterSwapper(masterSwapper.address, { gasPrice: 0 });
    await controller.connect(timelockController).editPriceOracle(priceOracle.address, { gasPrice: 0 });

    console.log('Test:: check 3');

    await proxyAdmin.upgrade(distributor.address, distributorNewImpl.address);
    console.log('Test:: check 4');
    ...
    console.log('Test:: check 9');

    await gardenBeacon.connect(owner).upgradeTo(gardenNewImpl.address);
    console.log('Test:: check 10');

    // Strategy to fix
    const strategies = ['0xcd9498b4160568DeEAb0fE3A0De739EbF152CB48'];
    const strategyContract = await ethers.getContractAt('IStrategy', strategies[0], owner);
    await strategyContract.connect(keeper).finalizeStrategy(0, '');
    console.log('rewards', (await strategyContract.strategyRewards()).toString());
  });
