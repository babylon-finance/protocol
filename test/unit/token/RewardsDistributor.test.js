...
(!!process.env.FAST ? describe.skip : describe)('RewardsDistributor', function () {
  ...
  describe('Strategy BABL Mining Rewards Calculation', async function () {
    ...
    it('should calculate correct BABL in case of 2 strategies with total duration of 1 quarter', async function () {
      ...
      await executeStrategy(long1, { amount: eth() });
      await executeStrategy(long2, { amount: eth().mul(2) });
    ...
    it('should calculate correct BABL in case of 3 strategies with total duration of 1 quarter', async function () {
      ...
      await executeStrategy(long1, { amount: eth() });
      await executeStrategy(long2, { amount: eth() });
      await executeStrategy(long3, { amount: eth() });
    ...
    it('should calculate correct BABL in case of 5 strategies of 2 different Gardens with total duration of less than 1 quarter', async function () {
      ...
      await executeStrategy(long2, { amount: eth() });
      await executeStrategy(long3, { amount: eth() });
      await executeStrategy(long4, { amount: eth() });
      await executeStrategy(long5, { amount: eth() });
    ...
    [
      ...
      pick([
        ...
        it(`should apply ${action} with new benchmark params in case of 1 strategy and total duration of 1 quarter in a ${name} garden`, async function () {
          ...
          await executeStrategy(long1, { amount: amount });
  ...
  describe('Claiming Reserve Asset Rewards and BABL Rewards', function () {
    ...
    it('rejects wrong nonce and updates it along the way', async function () {
      const amountIn = eth();
      const minAmountOut = eth('0.9');
      // Heart has a min contribution of 10 BABL
      // We use testGarden to use real BABL reserve asset (addresses.tokens.BABL)
      await rewardsDistributor.connect(owner).setHeartGarden(heartGarden.address);
      const reserveAssetContract = await getERC20(addresses.tokens.BABL);
      await fund([signer1.address, signer2.address], { tokens: [addresses.tokens.WETH, addresses.tokens.BABL] });
      const newGarden = await createGarden({ reserveAsset: addresses.tokens.WETH });
      await weth.connect(signer2).approve(newGarden.address, amountIn, {
        gasPrice: 0,
      });

      await reserveAssetContract.connect(signer2).approve(heartGarden.address, amountIn.mul(20), {
        gasPrice: 0,
      });
      await newGarden.connect(signer2).deposit(amountIn, minAmountOut, signer2.getAddress(), false);
      // We deposit some BABL to increase garden nonce into 1
      await heartGarden.connect(signer2).deposit(amountIn.mul(10), minAmountOut, signer2.getAddress(), false);
      const [long1] = await createStrategies([{ garden: newGarden }]);
      ...
      const stakeInHeart = false;
      // We prepare the call data
      // userRewardsNonce is 5 at this point, we pass 4 and it reverts
      expect(userRewardsNonce).to.eq(5);
      const userRewardsNonce1 = 4;
      // We create the signature
      let sig = await getRewardsSig(
        signer2,
        rewardsDistributor.address,
        totalBabl,
        totalProfits,
      ...
      const boolSignatureData = [mintNft, stakeInHeart];
      // approval for future keeper fee payment
      ...
      // Signer 2 claim rewards by sig
      const userRewardsNonce2 = 6;
      // We create the signature
      sig = await getRewardsSig(
        signer2,
        rewardsDistributor.address,
        totalBabl,
        totalProfits,
      ...
      // nonce is 6 at this point
      const [long2] = await createStrategies([{ garden: newGarden }]);
      // nonce is 7 at this point as there is a hidden deposit for signer2 while creating long2 strategy
      await executeStrategy(long2, { amount: eth() });
      await injectFakeProfits(long2, eth().mul(200));
      await finalizeStrategyAfterQuarter(long2);

      const rewardsAll2 = await rewardsAssistant.getAllUserRewards(signer2.address, [newGarden.address], {
        gasPrice: 0,
      });
      const gardensLast = rewardsAll2[0];
      const bablPerGardenLast = rewardsAll2[1];
      const profitsPerGardenLast = rewardsAll2[2];
      const totalBablLast = rewardsAll2[3];
      const totalProfitsLast = rewardsAll2[4];
      const userRewardsNonceLast = 7;
      const stakeMinAmountOutLast = minAmountOut;
      const contributorDataLast = await heartTestGarden.getContributor(signer2.address);
      const heartGardenUserNonceLast = contributorDataLast[9];
      ...
      sig = await getRewardsSig(
        ...
        totalBablLast,
        totalProfitsLast,
        userRewardsNonceLast,
        stakeMinAmountOutLast,
        heartGardenUserNonceLast,
        maxFee,
        mintNft,
        stakeInHeart,
      );
    ...
    it('can avoid replay attack between claimRewardsBySig and claimRewards', async function () {
      const amountIn = eth();
      const minAmountOut = eth('0.9');
      ...
      const stakeInHeart = false;
      // We prepare the call data
      // userRewardsNonce is 4 at this point, we pass 4 and it reverts
      expect(userRewardsNonce).to.eq(4);
      // We create the signature
      const sig = await getRewardsSig(
        signer2,
        rewardsDistributor.address,
      ...
      await rewardsDistributor.connect(signer2).claimRewards(gardens, stakeInHeart, stakeMinAmountOut, mintNft);
    ...
    it('can claimRewardsBySig all BABL and profit rewards from two different Gardens in only 1 claim tx by sig', async function () {
      ...
      const userGardens = rewardsAll[0];
      const bablPerGarden = rewardsAll[1];
      const profitsPerGarden = rewardsAll[2];
      const totalBabl = rewardsAll[3];
      const totalProfits = rewardsAll[4];
      // nonce is 4 as it deposited twice in 2 gardens before
      const userRewardsNonce = await rewardsDistributor.getUserRewardsNonce(signer3.address);
      const stakeMinAmountOut = eth(0.9);
      const contributorData = await heartTestGarden.getContributor(signer3.address);
      const heartGardenUserNonce = contributorData[9];
      const maxFee = 1;
      const fee = 1;
      const pricePerShare = await gardenValuer.calculateGardenValuation(heartTestGarden.address, bablToken.address);
      ...
      expect(await rewardsDistributor.getUserRewardsNonce(signer3.address)).to.be.eq(4);
      ...
      expect(await rewardsDistributor.getUserRewardsNonce(signer3.address)).to.be.eq(5);
    ...
    it('can claimRewards all BABL and profit rewards from two different Gardens in only 1 claim tx by sig', async function () {
      const [long1, long2, long3, long4, long5] = await createStrategies([
        { garden: garden1 },
        { garden: garden1 },
        { garden: garden2 },
      ...
      await rewardsDistributor
        .connect(signer1)
        .claimRewards([garden2.address, garden1.address], stakeInHeart, stakeMinAmountOut, mintNft, { gasPrice: 0 });
      const balanceAfter = await bablToken.balanceOf(signer1.address);
      expect(balanceAfter).to.be.gt(balanceBefore);
      expect(balanceAfter).to.eq(balanceBefore.add(rewardsSigner1Garden1[5]).add(rewardsSigner1Garden2[5]));
      expect(rewardsAll[3]).to.eq(balanceAfter.sub(balanceBefore));
    });
    ...
    it('should NOT get BABL rewards in a claim if it is not a contributor', async function () {
      ...
      // We claim our tokens and check that they are received properly
      await expect(
        rewardsDistributor.connect(signer3).claimRewards([garden1.address], stakeInHeart, stakeMinAmountOut, mintNft),
    ...
    it('should get (little) BABL rewards despite the user joined after the strategy execution (must join before strategy is exited anyway)', async function () {
      ...
      await injectFakeProfits(long1, eth().mul(200));
      await increaseTime(ONE_DAY_IN_SECONDS * 90);
      // Signer 3 deposits just before the strategy ends
      ...
      expect(balanceSigner1).to.be.gt(balanceSigner3.mul(3));
    });
    ...
    it('should claim and update balances of Signer1 in DAI Garden as contributor of 1 strategy with profit within a quarter', async function () {
      ...
      await rewardsDistributor
        .connect(signer1)
        .claimRewards([daiGarden.address], stakeInHeart, stakeMinAmountOut, mintNft);
      ...
      expect(signer1BalanceBABLAfter).to.be.closeTo(signer1BABL.add(signer1BalanceBABLBefore), eth('0.005'));
    ...
    it('should claim and update balances of Signer1 in USDC Garden as contributor of 1 strategy with profit within a quarter', async function () {
      ...
      await rewardsDistributor
        .connect(signer1)
        .claimRewards([usdcGarden.address], stakeInHeart, stakeMinAmountOut, mintNft);
      // Check remaining rewards for users (if any)
    ...
    it('should claim and update BABL Rewards of Signer1 in USDC Garden and DAI Garden as contributor of 2 strategies in 2 different gardens with profit within a quarter', async function () {
      ...
      // Receive BABL token after claim
      const signer1BalanceBABLAfter = await bablToken.balanceOf(signer1.address);
      expect(signer1BalanceBABLAfter).to.be.closeTo(
        signer1BABLUSDC.add(signer1BABLDAI).add(signer1BalanceBABLBefore),
        signer1BalanceBABLAfter.div(100),
      );
    ...
    it('should claim and update BABL Rewards of Signer1 in USDC Garden and DAI Garden as contributor of 2 strategies in 2 different gardens with profit below expected return within a quarter', async function () {
      ...
      const stakeMinAmountOut = eth(0.9);
      const mintNft = false;
      const signer1BalanceBefore = await bablToken.balanceOf(signer1.address);
      ...
      const signer1BalanceBABLAfter = await bablToken.balanceOf(signer1.address);
      expect(signer1BalanceBABLAfter).to.be.closeTo(
        signer1BABLUSDC.add(signer1BABLDAI).add(signer1BalanceBefore),
        eth('0.0005'),
      );
    });
    ...
    it('should not allow a race condition of two consecutive claims for the same rewards & profit of the same strategies', async function () {
      ...
      const signer1BABLBefore = await bablToken.balanceOf(signer1.address);
      const signer2BABLBefore = await bablToken.balanceOf(signer2.address);
      ...
      await expect(
        rewardsDistributor.connect(signer1).claimRewards([garden1.address], stakeInHeart, stakeMinAmountOut, mintNft),
      ).to.be.revertedWith('BAB#082');
      ...
      await expect(
        rewardsDistributor.connect(signer2).claimRewards([garden1.address], stakeInHeart, stakeMinAmountOut, mintNft),
      ).to.be.revertedWith('BAB#082');
      ...
      expect(await bablToken.balanceOf(signer1.address)).to.be.closeTo(signer1BABL.add(signer1BABLBefore), eth('0.05'));
      expect(await bablToken.balanceOf(signer2.address)).to.be.closeTo(signer2BABL.add(signer2BABLBefore), eth('0.05'));
    ...
    it('should only provide new additional BABL and profits between claims (claiming results of 2 strategies only 1 with profit)', async function () {
      ...
      expect(await bablToken.balanceOf(signer1.address)).to.be.closeTo(
        signer1BABL.add(signer1BABLBefore),
        eth('0.005'),
      );
      expect(signer1Profit.toString()).to.be.closeTo('11230104410277293', eth('0.005'));
      const signer1Rewards2 = await rewardsDistributor.getRewards(garden1.address, signer1.address, [
        long1.address,
        long2.address,
      ]);
    ...
    it('should only provide new additional BABL and profits between claims (claiming results of 2 strategies both with profit)', async function () {
      ...
      expect((await bablToken.balanceOf(signer1.address)).toString()).to.be.closeTo(
        signer1BABL.add(signer1BABL2).add(signer1BABLBefore),
        eth('0.02'),
      );
    ...
    it('should claim and update balances of Signer1 either Garden tokens or BABL rewards as contributor of 5 strategies (4 with positive profits) of 2 different Gardens with different timings along 3 Years', async function () {
      ...
      expect(await bablToken.balanceOf(signer2.address)).to.be.closeTo(
        signer2BABL.add(signer2BABLBefore),
        eth('0.0005'),
      );
      expect(signer1Profit.toString()).to.be.closeTo('5273755118202756', eth('0.0005'));
      ...
      await rewardsDistributor
        .connect(signer1)
        .claimRewards([garden2.address], stakeInHeart, stakeMinAmountOut, mintNft);
      await rewardsDistributor
        .connect(signer2)
        .claimRewards([garden2.address], stakeInHeart, stakeMinAmountOut, mintNft);

      expect(await bablToken.balanceOf(signer1.address)).to.be.closeTo(
        signer1BABL2.add(signer1BABL).add(signer1BABLBefore),
        eth('0.0005'),
      );
      expect(await bablToken.balanceOf(signer2.address)).to.be.closeTo(
        signer2BABL2.add(signer2BABL).add(signer2BABLBefore),
        eth('0.0005'),
      );
      ...
      expect(signer1Profit2.toString()).to.be.closeTo('17365212142975108', eth('0.00005'));
      expect(signer2Profit2.toString()).to.be.closeTo('3142522732687342', eth('0.00005'));
    });
  ...
  describe('NFT stake in Gardens to boost BABL rewards', function () {
    it('can stake common prophet NFT in a garden to get 1% LP', async function () {
      ...
      expect(ts).to.eq(0);
      const [long1, long2] = await createStrategies([{ garden: garden1 }, { garden: garden1 }]);
      await executeStrategy(long1, { amount: eth().mul(2) });
      await increaseTime(ONE_DAY_IN_SECONDS * 30);
      await finalizeStrategyImmediate(long1);
