...
const { WETH_STRATEGY_PARAMS } = require('fixtures/StrategyHelper');
...
describe('Babylon Viewer', function () {
  ...
  describe('can call getter methods', async function () {
    ...
    it('calls get complete strategy', async function () {
      ...
      expect(strategyDetails[2][1]).to.equal(WETH_STRATEGY_PARAMS[1]); // Stake
      expect(strategyDetails[2][2]).to.equal(WETH_STRATEGY_PARAMS[1]); // Positive votes
      ...
      expect(strategyDetails[2][6]).to.equal(WETH_STRATEGY_PARAMS[2]); // Duration
      expect(strategyDetails[2][7]).to.equal(WETH_STRATEGY_PARAMS[3]); // Expected Return
      expect(strategyDetails[2][8]).to.equal(WETH_STRATEGY_PARAMS[0]); // Max Capital Requested
      ...
      expect(strategyDetails[2][12]).to.equal(WETH_STRATEGY_PARAMS[4]); // Max Allocation Percentage
