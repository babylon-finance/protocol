...
describe("ReservePool", function() {
  ...
  describe("Deposit", async function() {
    ...
    it("can deposit and receive RBABL tokens", async function() {
      ...
      await expect(await weth.balanceOf(reserve.address)).to.equal(
        ethers.utils.parseEther("1")
      );
      await expect(await reserve.totalSupply()).to.equal(
        ethers.utils.parseEther("1")
      );
      await expect(await reserve.balanceOf(userSigner2.address)).to.equal(
        ethers.utils.parseEther("1")
      );
      await expect(await reserve.getReservePoolValuation()).to.equal(
        ethers.utils.parseEther("1")
      );
  ...
  describe("Claim", async function() {
    it("cannot claim before the timelock window expires", async function() {
      ...
      await expect(
        reserve
          .connect(userSigner2)
          .claim(ethers.utils.parseEther("1"), userSigner2.address)
      ).to.be.reverted;
    });
    ...
    it("cannot claim after the timelock window more than the deposited", async function() {
      ...
      await expect(
        reserve
          .connect(userSigner2)
          .claim(ethers.utils.parseEther("1.01"), userSigner2.address)
      ).to.be.reverted;
    });
    ...
    it("can claim and redeem RBABL tokens", async function() {
      ...
      await reserve
        .connect(userSigner2)
        .claim(ethers.utils.parseEther("1"), userSigner1.address);
      const supplyBefore = await reserve.totalSupply();
      expect(supplyBefore).to.equal(0);
    });
  });
