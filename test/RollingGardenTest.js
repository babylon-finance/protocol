...
describe("Garden", function() {
  ...
  describe("Garden construction", async function() {
    it("should have expected properties upon deployment", async function() {
      ...
      expect(await garden1.creator()).to.equal(
        await userSigner1.getAddress()
      );
      expect(await garden1.controller()).to.equal(controller.address);
      expect(await garden1.strategyCooldownPeriod()).to.equal(
        ONE_DAY_IN_SECONDS
      );
      expect(await garden1.strategyCreatorProfitPercentage()).to.equal(
        ethers.utils.parseEther("0.13")
      );
      expect(await garden1.strategyVotersProfitPercentage()).to.equal(
        ethers.utils.parseEther("0.05")
      );
      expect(await garden1.gardenCreatorProfitPercentage()).to.equal(
        ethers.utils.parseEther("0.02")
      );
      expect(await garden1.minVotersQuorum()).to.equal(
        ethers.utils.parseEther("0.10")
      );
      expect(await garden1.minIdeaDuration()).to.equal(
        ONE_DAY_IN_SECONDS * 3
      );
      expect(await garden1.maxIdeaDuration()).to.equal(
  ...
  describe("Garden state", async function() {
    it("only the protocol should be able to update active state", async function() {
      await expect(garden1.connect(userSigner1).setActive(true)).to.be
        .reverted;
    });
  ...
  describe("Garden contributors", async function() {
    it("a contributor can make an initial deposit", async function() {
      ...
      expect(gardenBalanceAfter.sub(gardenBalance)).to.equal(
        ethers.utils.parseEther("1")
      );
      expect(await garden1.totalContributors()).to.equal(2);
      expect(await garden1.getPrincipal()).to.equal(
      ...
      // Contributor Struct
      const contributor = await garden1.contributors(
    ...
    it("a contributor cannot make a deposit when the garden is disabled", async function() {
      await expect(controller.disableGarden(garden1.address)).to.not.be
        .reverted;
      await expect(
        garden1
          .connect(userSigner3)
