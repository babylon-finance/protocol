...
abstract contract BorrowIntegration is BaseIntegration, ReentrancyGuard, IBorrowIntegration {
    ...
    function _validatePostBorrow(DebtInfo memory _debtInfo) internal view {
        uint256 balance = address(0) == _debtInfo.asset ? address(_debtInfo.strategy).balance : IERC20(_debtInfo.asset).balanceOf(address(_debtInfo.strategy));
        require(
             balance >= _debtInfo.amount,
