...
contract GovernorBravoDelegate is GovernorBravoDelegateStorageV1, GovernorBravoEvents {
    ...
    string public constant name = 'Babylon Finance Governor Bravo';

    /// @notice The minimum setable proposal threshold
    uint256 public constant MIN_PROPOSAL_THRESHOLD = 5000e18; // 5,000 BABL

    /// @notice The maximum setable proposal threshold
    uint256 public constant MAX_PROPOSAL_THRESHOLD = 10000e18; //10,000 BABL
    ...
    uint256 public constant quorumVotes = 40000e18; // 40,000 = 4% of BABL
     ...
     * @param babl_ The address of the BABL token
     * @param votingPeriod_ The initial voting period
     * @param votingDelay_ The initial voting delay
     * @param proposalThreshold_ The initial proposal threshold
     */
    // TODO: MAKE IT UPGRADEABLE AND AVOID 2 INITIALIZATIONS
    function initialize(
        address timelock_,
        address babl_,
        ...
        require(babl_ != address(0), 'GovernorBravo::initialize: invalid babl address');
        require(
    ...
    function initiate() external {
        require(msg.sender == admin, 'GovernorBravo::initiate: admin only');
        require(initialProposalId == 0, 'GovernorBravo::initiate: can only initiate once');
        proposalCount = 0;
    ...
    function propose(
        ...
        require(
            babl.getPriorVotes(msg.sender, _sub256(block.number, 1)) > proposalThreshold,
    ...
    /**
     * PRIVILEGED GOVERNANCE FUNCTION. Allows the cancellation of a proposal if state != executed
     *
    ...
    function cancel(uint256 proposalId) external {
        ...
        require(
            msg.sender == proposal.proposer ||
                babl.getPriorVotes(proposal.proposer, _sub256(block.number, 1)) < proposalThreshold,
     ...
     *
     * @notice Allows the caller to get the list of actions of a proposal
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        ...
        } else if (block.timestamp >= _add256(proposal.eta, timelock.GRACE_PERIOD())) {
    ...
    /**
     * GOVERNANCE FUNCTION. Allows the msg.sender to cast a vote with a reason
     *
    ...
    /**
     * GOVERNANCE FUNCTION. Sets a new proposal threshold
     *
    ...
    function setProposalThreshold(uint256 newProposalThreshold) external {
        require(msg.sender == admin, 'GovernorBravo::setProposalThreshold: admin only');
        require(
    ...
    /**
     * GOVERNANCE FUNCTION. Allows to set the voting delay
     *
     * @notice Admin function for setting the voting delay
     * @param newVotingDelay new voting delay, in blocks
     */
    function setVotingDelay(uint256 newVotingDelay) external {
        require(msg.sender == admin, 'GovernorBravo::setVotingDelay: admin only');
        require(
    ...
    /**
     * GOVERNANCE FUNCTION. Allows to set a voting period
     *
     * @notice Admin function for setting the voting period
     * @param newVotingPeriod new voting period, in blocks
     */
    function setVotingPeriod(uint256 newVotingPeriod) external {
        require(msg.sender == admin, 'GovernorBravo::setVotingPeriod: admin only');
        require(
    ...
    /**
     * PRIVILEGED GOVERNANCE FUNCTION. Sets the pending admin
     *
    ...
    function setPendingAdmin(address newPendingAdmin) external {
        // Check caller = admin
        require(msg.sender == admin, 'GovernorBravo:setPendingAdmin: admin only');
    ...
    /**
     * PRIVILEGED GOVERNANCE FUNCTION. Accepts transfer of admin rights. msg.sender must be pendingAdmin
     *
    ...
    function _queueOrRevertInternal(
        ...
        require(
            ...
            'GovernorBravo::_queueOrRevertInternal: identical proposal action already queued at eta'
        );
        timelock.queueTransaction(target, value, signature, data, eta);
    }

    /**
     * GOVERNANCE FUNCTION. Allows to cast a vote
     *
    ...
    function _castVoteInternal(
        ...
        require(support <= 2, 'GovernorBravo::_castVoteInternal: invalid vote type');
        Proposal storage proposal = proposals[proposalId];
        ...
        if (support == 0) {
            ...
            proposal.abstainVotes = _add256(proposal.abstainVotes, votes);
        }
     ...
     */
    function _getChainIdInternal() internal pure returns (uint256) {
        uint256 chainId;
        assembly {
            chainId := chainid()
        }
