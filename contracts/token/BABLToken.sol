...
contract BABLToken is TimeLockedToken {
  ...
  function mint(address _to, uint256 _amount) external onlyOwner returns (bool) {
    ...
    require(amount <= totalSupply().mul(MINT_CAP).div(100), 'BABLToken::mint: exceeded mint cap of 2% of total supply');
    _mint(_to, amount);
  ...
  function changeMaxSupply(uint256 newMaxSupply, uint256 newMaxSupplyAllowedAfter) external onlyOwner returns (bool) {
    ...
    uint96 time =
      safe96(newMaxSupplyAllowedAfter, 'BABLToken::changeMaxSupply: new newMaxSupplyAllowedAfter exceeds 96 bits'); // Overflow check
    uint96 futureTime =
  ...
  function maxSupply() external view returns (uint96, uint256) {
    uint96 safeMaxSupply = safe96(maxSupplyAllowed, 'BABLToken::maxSupplyAllowed: maxSupplyAllowed exceeds 96 bits'); // Overflow check
