...
contract Garden is ERC20Upgradeable, ReentrancyGuard, IGarden {
  ...
  function allocateCapitalToStrategy(uint256 _capital) external override {
    ...
    _require(
      _capital.add(protocolMgmtFee) <= IERC20(reserveAsset).balanceOf(address(this)).sub(reserveAssetPrincipalWindow),
      Errors.MIN_LIQUIDITY
    );
   ...
   */
  function getExpectedReserveWithdrawalQuantity(uint256 _gardenTokenQuantity) external view override returns (uint256) {
  ...
  function getGardenTokenMintQuantity(
    ...
    uint256 gardenValuationPerToken =
      IGardenValuer(IBabController(controller).gardenValuer()).calculateGardenValuation(address(this), reserveAsset);
    if (isDeposit) {
  ...
  function _onlyUnpaused() private view {
    ...
    _require(
      !IBabController(controller).guardianGlobalPaused() && !IBabController(controller).guardianPaused(address(this)),
  ...
  function _withdraw(
    ...
    _require(block.timestamp.sub(contributors[msg.sender].lastDepositAt) >= depositHardlock, Errors.DEPOSIT_HARDLOCK);
    _reenableReserveForStrategies();
    ...
    returns (uint256)
  {
    ...
    // Reverts if price is not found
    uint256 gardenValuationPerToken =
      IGardenValuer(IBabController(controller).gardenValuer()).calculateGardenValuation(address(this), _reserveAsset);
