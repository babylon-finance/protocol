...
contract DepositVaultOperation is Operation {
                ...
                try IPassiveIntegration(_integration).getRewards(msg.sender, yieldVault) returns (
                    ...
                    if (rewardToken != address(0)) {
                        ...
                        if (amount > MIN_TRADE_AMOUNT) {
                            ...
                            IStrategy(msg.sender).trade(rewardToken, amount,
                                                        rasset, 0, IStrategy.TradeInfo(address(0), address(0), 0, address(0), 0, address(0))  );
                        }
                    }
                } catch {}
            }
        ...
        )
    {
        uint256 vaultAssetQuantity =
            _vaultAsset != _asset
                ? IStrategy(msg.sender).trade(_asset, _capital, _vaultAsset, 0, IStrategy.TradeInfo(address(0), address(0), 0, address(0), 0, address(0)) )
