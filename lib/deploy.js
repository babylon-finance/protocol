...
export async function getContracts() {
  ...
  const tokenIdentifier = await ethers.getContractAt(
    'TokenIdentifier',
    '0x21c056b317037054fd78940F9ADea338B6668DC8',
    owner,
  );
...
const deployFixture = deployments.createFixture(async (hre, options) => {
  ...
  const tokenIdentifier = await deploy('TokenIdentifier', {
    ...
    args: [
      controller.address,
      (await deployments.get('IPickleJarRegistry')).address,
      (await deployments.get('IYearnVaultRegistry')).address,
      curveMetaRegistry.address,
      (await deployments.get('IConvexRegistry')).address,
    ],
  });
  ...
  for (const { contract, type, operation, args } of [
    ...
    },
    {
      ...
      type: 'integration',
      args: [controller.address, '0xF8B6d144d7b0026980b7ef1E488F6F09Ad426F49'],
    },
