...
export async function getContracts() {
  ...
  return { gov, owner, deployer, keeper, controller, distributor, strategyNft, valuer, gardens };
}

const deployFixture = deployments.createFixture(async (hre, options) => {
  const { ethers } = hre;

  const { owner, gov, deployer, keeper, controller, distributor, strategyNft, valuer, gardens } = await getContracts();

  await fund([owner.address, deployer.address, TIMELOCK_ADDRESS], {
  ...
  for (const garden of gardens) {
    ...
    /*     console.log('garden nav', garden, gardenNAV.toString());
     */
  }
  // upgrade controller
  ...
  return { controller, distributor, owner, gov, deployer, keeper, gardens, gardensNAV, strategyNft, valuer };
});
