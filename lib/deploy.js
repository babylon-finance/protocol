...
const deployFixture = deployments.createFixture(async (hre, options) => {
  ...
  const viewerDeployment = await deploy('BabylonViewer', {
    from: signer.address,
    args: [controller.address],
    log: true,
    maxPriorityFeePerGas,
  });
  console.log('deploy:: check 6');

  const babViewer = await ethers.getContractAt('BabylonViewer', viewerDeployment.address);
