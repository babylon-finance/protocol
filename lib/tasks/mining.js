...
task('mining').setAction(async (args, { getContract, ethers, getGasPrice }, runSuper) => {
  ...
  let totalHeartGardenBABLRewards = from(0); // Heart Garden Rewards from finalized strategies
  let totalHeartGardenEstimateBABLRewards = from(0); // Heart Garden Rewards from executing strategies
  for (const garden of gardens) {
    ...
    for (const strategy of finalizedStrategies) {
      ...
      if (finalized) {
        ...
        if (garden === heartGarden.address) {
          totalHeartGardenBABLRewards = totalHeartGardenBABLRewards.add(rewards);
        }
    ...
    for (const strategy of strategies) {
      ...
      if (executedAt.gt(0) && !finalized) {
        ...
        gardenBaselineRewards = gardenBaselineRewards.add(baselineRewards);
        if (garden === heartGarden.address) {
          totalHeartGardenEstimateBABLRewards = totalHeartGardenEstimateBABLRewards.add(strategyEstimateRewards);
        }
