...
task('mining').setAction(async (args, { getContract, ethers, getGasPrice }, runSuper) => {
  ...
  console.log(claimedBABL.toString());
  console.log('');
  console.log('');
  console.log('Babylon Protocol Mining Performance');
  console.log('');
  console.log(` Supply performance:`);
  console.log(`  Rewards Distributor BABL balance: ${chalk.green(normalizeToken(remainingBABL))} BABL 💰`);
  console.log(`  BABL Claimed: ${chalk.red(normalizeToken(claimedBABL))} BABL 💰`);
  console.log(
    `  BABL Available supply: ${chalk.green(
      normalizeToken(remainingBABL.add(claimedBABL).sub(totalRealRewards)),
    )} BABL 💰`,
  );
  console.log(`  BABL Exposed supply (baseline): ${chalk.blueBright(normalizeToken(totalBaseline))} BABL 💰`);
  console.log('');
  console.log(' Real delivery performance:');
  console.log(
    `  Total BABL unclaimed (real): ${chalk.green(normalizeToken(totalRealUnclaimedRewards))} BABL (${chalk.green(
      normalizeToken(totalUnclaimedRewards),
    )})`,
  );
  console.log(
    `  Total BABL pending (real): ${chalk.cyan(normalizeToken(totalRealEstimateRewards))} BABL (${chalk.cyan(
      normalizeToken(totalEstimateRewards),
    )})`,
  );
  console.log(
    `  Grand Total BABL (real): ${chalk.blue(normalizeToken(totalRealRewards))} BABL 💰 (${chalk.blue(
      normalizeToken(totalRewards),
    )})`,
  );
  console.log('');
