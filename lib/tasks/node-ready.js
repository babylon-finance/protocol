...
subtask(TASK_NODE_SERVER_READY).setAction(async (args, hre, runSuper) => {
  ...
  await exportContracts(
    [
      ...
      'strategies/LendStrategy.sol/LendStrategy.json',
      'strategies/LiquidityPoolStrategy.sol/LiquidityPoolStrategy.json',
      'strategies/LongStrategy.sol/LongStrategy.json',
      'strategies/YieldFarmingStrategy.sol/YieldFarmingStrategy.json',
  ...
  if (exists) {
    await fs.copy('contracts.json', '../dapp/contracts.json');
  }
});
