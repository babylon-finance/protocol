...
task('tvl').setAction(async (args, { getContract, ethers, getGasPrice }, runSuper) => {
  ...
  for (const garden of gardens) {
    ...
    console.log(
      `  Rewards: $${chalk.green(formatUnit(reserveAssetRewardsSetAside))}`,
    );
    console.log(
      `  Absolute Returns: $${chalk.green(formatUnit(absoluteReturns))}`,
    );
    console.log(
      `  Expected Returns: $${chalk.green(formatUnit(expectedReturns))}`,
    );
