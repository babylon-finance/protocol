...
task('upgrade-beacon')
  ...
  .setAction(
    async (
      ...
      const controller = await upgradesDeployer.upgradeBeacon(beacon, from, to, undefined, { from: signer.address, log: true, gasPrice });

      console.log(`Uploading contract to Tenderly`);

      if (network.live && controller.newlyDeployed) {
        const contract = await getTenderlyContract(['BabController']);
