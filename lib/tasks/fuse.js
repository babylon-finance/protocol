...
task('fuse')
  ...
  .setAction(async (args, { getContract, ethers, getGasPrice }, runSuper) => {
    ...
    console.log('');
    console.log(`Stats for Stable Garden`);
    strategyNft = await getContract('StrategyNFT', 'StrategyNFT', deployer);
    const stableGarden = await ethers.getContractAt('IGarden', '0x1D50c4F18D7af4fCe2Ea93c7942aae6260788596');
    strategies = await stableGarden.getStrategies();
    finalizedStrategies = await stableGarden.getFinalizedStrategies();
    ...
    console.log('');
    console.log(`Stats for Fountain of ETH Garden`);
    strategyNft = await getContract('StrategyNFT', 'StrategyNFT', deployer);
    const fountainETHGarden = await ethers.getContractAt('IGarden', '0xB5bD20248cfe9480487CC0de0d72D0e19eE0AcB6');
    strategies = await fountainETHGarden.getStrategies();
    finalizedStrategies = await fountainETHGarden.getFinalizedStrategies();
    ...
    console.log('');
    console.log(`Stats for Stable Peeble Garden`);
    strategyNft = await getContract('StrategyNFT', 'StrategyNFT', deployer);
    const stablePeebleGarden = await ethers.getContractAt('IGarden', '0x3eeC6Ac8675ab1B4768f6032F0598e36Ac64f415');
    strategies = await stablePeebleGarden.getStrategies();
    finalizedStrategies = await stablePeebleGarden.getFinalizedStrategies();
    ...
    console.log('');
    console.log(`Stats for Waterfall2 Garden`);
    strategyNft = await getContract('StrategyNFT', 'StrategyNFT', deployer);
    const waterfall2Garden = await ethers.getContractAt('IGarden', '0x99acDD18eb788E199be6Bf64d14142329316687a');
    strategies = await waterfall2Garden.getStrategies();
    finalizedStrategies = await waterfall2Garden.getFinalizedStrategies();
