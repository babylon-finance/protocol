...
const FEI = '0x3a2804ec0Ff521374aF654D8D0daA1d1aE1ee900';
...
task('fuse')
  ...
  .setAction(async (args, { getContract, ethers, getGasPrice }, runSuper) => {
    async function getFuseStatsForAddress({ address, comptroller, bablPrice, log, prefix, name }) {
      ...
      if (totalCollateral.gt(MIN_COLLATERAL)) {
        console.log('');
        ...
        // console.log(`  ${prefix} ${name} at ${address}`);
        // console.log(`  Skipped due to low Collateral: $${formatUnit(totalCollateral)}`);
      }
    }
    ...
    let name = '';
    await getFuseStatsForAddress({ address: HEART, comptroller, bablPrice, log, prefix: HEART_PREFIX, name });
    console.log(``);
    ...
    const finalizedStrategies = await heartGarden.getFinalizedStrategies();

    for (const strategy of strategies) {
      const name = await strategyNft.getStrategyName(strategy);
      await getFuseStatsForAddress({ address: strategy, comptroller, bablPrice, log, prefix: ACTIVE_STR_PREFIX, name });
    }
