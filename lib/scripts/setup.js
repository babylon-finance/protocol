...
async function setup(hre) {
  ...
  const viewer = await ethers.getContractAt('IViewer', (await deployments.get('Viewer')).address);
  ...
  console.log('**** HEART GARDEN ***', gardenBabl);

  // Set the heart
  await heart.connect(owner).setHeartGardenAddress(gardenBabl, { gasPrice: 0 });
  ...
  console.log('Adding bonds...');
  const heartProd = await ethers.getContractAt('Heart', await viewer.heart());
  addresses.bonds.forEach(async (bond) => {
    console.log('bond adding', bond.address, heart.address, heartProd.address);
    await heart.connect(owner).updateBond(bond.address, eth(0.05), { gasPrice: 0 });
    await heartProd.connect(owner).updateBond(bond.address, eth(0.05), { gasPrice: 0 });
  });
