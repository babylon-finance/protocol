...
async function setup(hre) {
  ...
  // We use the local deployed BABLToken for tests
  // const bablToken = await ethers.getContractAt('BABLToken', addresses.tokens.BABL);
  const bablToken = await ethers.getContractAt('BABLToken', (await deployments.get('BABLToken')).address);

  const rewardsDistributor = await getContract('RewardsDistributor', 'RewardsDistributorProxy');
  console.log('BABLToken', bablToken);
  console.log('RD', rewardsDistributor);
  ...
  console.log('Giving Local BABL...');
  // fund with local hardhat BABL Token created to create a Test Heart Garden of local BABL reserveAsset
  const treasurySigner = await impersonateAddress((await deployments.get('Treasury')).address);
  ...
  const realBabl = await getERC20(addresses.tokens.BABL);
  console.log('Real BABL balance', ethers.utils.formatEther(await realBabl.balanceOf(signer1.address)));
  await realBabl.connect(signer1).approve(babController.address, await realBabl.balanceOf(signer1.address), {
    gasPrice: 0,
  });
  await babController
    .connect(signer1)
  ...
  /*
  console.log('creating Heart Test BABL garden');
  console.log('Local hardhat BABL balance', ethers.utils.formatEther(await bablToken.balanceOf(signer1.address)));
  ...
  const heartTestGarden = await ethers.getContractAt('Garden', gardens[0]);
  ...
  await rewardsDistributor.connect(owner).setHeartGarden(heartTestGarden.address, { gasPrice: 0 });
