...
async function setup(hre) {
  ...
  const gardens = (await babController.getGardens()).slice(-3);

  await babController.connect(owner).setAllowPublicGardens({ gasPrice: 0 });
  ...
  await fund(
    ...
    {
      tokens: [addresses.tokens.USDC, addresses.tokens.DAI, addresses.tokens.WETH],
    },
  );
