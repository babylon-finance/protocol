...
async function setup(hre) {
  ...
  const [deployer, keeper, owner, signer1, signer2, signer3, signer4, signer5, signer6, signer7, signer8] =
  ...
  const babController = await ethers.getContractAt(
    'BabController',
    (
      await deployments.get('BabControllerProxy')
    ).address,
  );
  ...
  const uniswapV3TradeIntegration = await ethers.getContractAt(
    ...
    (
      await deployments.get('UniswapV3TradeIntegration')
    ).address,
  );
