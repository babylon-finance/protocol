const { ONE_DAY_IN_SECONDS, GARDEN_PARAMS } = require('../constants.js');
...
async function setup(hre) {
  ...
  console.log('deployer', deployer.address, keeper.address);
  console.log('owner', owner.address, await ishtarGate.owner());
  // Gives signer1 creator permissions
  await ishtarGate.connect(deployer).setCreatorPermissions(signer1.address, true, { gasPrice: 0 });
  await ishtarGate.connect(deployer).setCreatorPermissions(owner.address, true, { gasPrice: 0 });
  await ishtarGate.connect(deployer).setCreatorPermissions(deployer.address, true, { gasPrice: 0 });

  // Give assigned wallets creator permission
  await ishtarGate.connect(deployer).setCreatorPermissions(signer4.address, true, { gasPrice: 0 });
  await ishtarGate.connect(deployer).setCreatorPermissions(signer5.address, true, { gasPrice: 0 });
  await ishtarGate.connect(deployer).setCreatorPermissions(signer6.address, true, { gasPrice: 0 });
  await ishtarGate.connect(deployer).setCreatorPermissions(signer7.address, true, { gasPrice: 0 });
  await ishtarGate.connect(deployer).setCreatorPermissions(signer8.address, true, { gasPrice: 0 });
  ...
  await babController.connect(deployer).setAllowPublicGardens();
  ...
  const wethWhaleAddress = getAssetWhale(addresses.tokens.WETH);
  ...
  console.log('weth whale signer', wethWhaleSigner.address);
