...
const { increaseTime } = require('../../test/utils/test-helpers.js');

async function setup(hre) {
  const { fund } = require('../whale');
  const { impersonateAddress } = require('../rpc');
  const { getERC20, eth, from } = require('utils/test-helpers');
  const { ethers } = hre;
  ...
  await executeStrategy(strategy1, keeper);

  await injectFakeProfits(strategy1, eth('5000'));
  await increaseTime(ONE_DAY_IN_SECONDS * 90);
  await finalizeStrategy(strategy1, keeper, { time: ONE_DAY_IN_SECONDS.mul(ethers.BigNumber.from(30)) });
