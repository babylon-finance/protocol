...
const MIN_LIQUIDITY_USD = 1 * 1000 * 1000; // 1MM
...
async function _processToken(token, curveRegistry, priceOracle, babylonViewer, uniV2Factory) {
...
async function _dedupeTokens(tokens) {
  const toProcess = tokens.slice(0, 1000);
  const seen = new Set();
...
async function buildTokenList(ethers) {
  const deduped = await _dedupeTokens(TOKENS);
  ...
  const processTokenWrapped = async (token) => {
    try {
      return await _processToken(token, curveRegistry, priceOracle, babylonViewer, uniV2Factory).catch(() => {
  ...
  let results = await Promise.map(deduped, processTokenWrapped, { concurrency: 100 }).then((output) => output);
