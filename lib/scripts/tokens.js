...
async function _getUniPool(address, priceOracle) {
  // UniV3
  try {
    const maybeWETHV3 = await priceOracle.getUNIV3Price(address, addresses.tokens.WETH);
  ...
  // UniV2
  try {
    const maybeWETHV2 = await priceOracle.getUNIV2Price(address, addresses.tokens.WETH);
...
async function _dedupeAndGetBestImage(tokens) {
  ...
  console.log(`Filtered ${tokens.length - filtered.length} duplicates!`);
...
async function _getLiquidity(address, babylonViewer) {
  try {
...
async function exportTokenList(ethers) {
  ...
  const promises = deduped.slice(0, 100).map(async (token) => {
    ...
    const liquidity = await _getLiquidity(token.address, babylonViewer);
