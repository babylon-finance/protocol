...
async function _sourceIntegration(address) {
  ...
  // Yearn
  if (
    Object.values(addresses.yearn.vaults)
      .map((t) => t.vault.toLowerCase())
      .includes(formattedAddress)
  ) {
...
async function _getV3Liquidity(address, viewer) {
  const formattedAddress = address.toLowerCase();

  if (!formattedAddress) {
    console.error(`Address missing for V3 liquidity check, skipping...`);
  }
  ...
  const promises = Object.entries(Reserves).map(async (reserve) => {
    ...
    if (formattedAddress !== details.address.toLowerCase()) {
      try {
        ...
        const result = await viewer.getPriceAndLiquidity(address, details.address);
...
async function _getBestV2Liquidity(token, uniV2Factory, priceOracle, ethers) {
  ...
  if (!formattedAddress) {
    console.error(`Address missing for token ${token.symbol}, skipping...`);
  }
...
async function _getViewer(ethers) {
  const contractObj = JSON.parse(fs.readFileSync(`./deployments/artifacts/mainnet/Viewer.json`));
  return await ethers.getContractAt('IViewer', contractObj.address);
}
...
async function _processToken(token, curveRegistry, priceOracle, viewer, uniV2Factory, progress) {
  ...
  try {
    ...
    if (!maybeIntegration) {
      const maybeV3Liquidity = await _getV3Liquidity(token.address, viewer);
...
async function buildTokenList(ethers, tokens, progress) {
  ...
  const viewer = await _getViewer(ethers);
  const uniV2Factory = await _getUniV2Factory(ethers);

  const processTokenWrapped = async (token) => {
    try {
      return await _processToken(token, curveRegistry, priceOracle, viewer, uniV2Factory, progress).catch(() => {
  ...
  const errors = [];
  let results = await Promise.map(tokens, processTokenWrapped, { concurrency: 25 }).then((output) => output);
