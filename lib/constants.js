...
function getGardenParams({
  ...
  return [
    !!maxDepositLimit ? maxDepositLimit : eth(20), // Max Deposit Limit
    !!minLiquidityAsset ? minLiquidityAsset : eth(100), // Min Liquidity Asset | ie: Uniswap Volume
    !!depositHardlock ? depositHardlock : 1, // Deposit Hardlock | 1 second
    !!minContribution ? minContribution : eth(0.1), // Min Contribution
    !!strategyCooldownPeriod ? strategyCooldownPeriod : ONE_DAY_IN_SECONDS, // Strategy Cooldown Period
    !!minVotesQuorum ? minVotesQuorum : eth(0.1), // Min Voter Quorum | 10%
    !!minStrategyDuration ? minStrategyDuration : ONE_DAY_IN_SECONDS * 3, // Min Strategy Duration
    !!maxStrategyDuration ? maxStrategyDuration : ONE_DAY_IN_SECONDS * 365, // Max Strategy Duration
    !!minVoters ? minVoters : 1, // Min number of voters
    !!pricePerShareDecayRate ? pricePerShareDecayRate : eth(), // Decay rate of price per share
    !!pricePerShareDelta ? pricePerShareDelta : eth(), //Base slippage for price per share
  ];
}
...
const STRATEGY_EXECUTE_MAP = {
  ...
  [addresses.tokens.DAI]: eth(2500),
  [addresses.tokens.BABL]: eth(60),
  [addresses.tokens.AAVE]: eth(30),
  [addresses.tokens.USDC]: from(2500 * 1e6),
...
const STRATEGY_PARAMS = {
  ...
  maxTradeSlippagePercentage: eth(0.05),
};
